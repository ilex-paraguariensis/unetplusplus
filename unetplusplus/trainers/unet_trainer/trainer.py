        ...
        with tqdm(total=n_train, desc=f"Epoch {epoch}/{epochs}", unit="img") as pbar:
            for batch in train_loader:
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    pred_masks = net(images).squeeze(1)
                    # print(f"{pred_masks.shape=}")
                    # print(f"{true_masks.shape=}")
                    # print(f"{pred_masks.dtype=}")
                    # print(f"{true_masks.dtype=}")
                    # print(f"{pred_masks[:, :5, :5]=}")
                    # print(f"{true_masks[:, :5, :5]=}")
                    reg_loss = criterion(pred_masks, true_masks)
                    shape_loss = dice_loss(
                        F.softmax(pred_masks, dim=1).float(),
                        F.one_hot(true_masks.long(), net.n_classes)
                        .permute(0, 3, 1, 2)
                        .float(),
                        multiclass=True,
                    )
                    loss = reg_loss + shape_loss
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        val_score = evaluate(net, val_loader)
                        ...
                        experiment.log(
                            {
                                ...
                                "masks": {
                                    ...
                                    "pred": wandb.Image(
                                        pred_masks.argmax(dim=1)[0].float().cpu()
                                    ),
                                },
                                "step": global_step,
                                "epoch": epoch,
